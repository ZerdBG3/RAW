...
local function RAW_AddAttunement(item)
    ...
    -- Attunement items can't be equipped in combat, except thrown weapons
    local combatRestriction = "((Player(context.Source) and not Combat(context.Source)) or not Player(context.Source)) and "
    if item.Slot == "Melee Main Weapon" and RAW_HasValueInList(item["Weapon Properties"], "Thrown") then
        combatRestriction = ""
    end
    ...
    item.PassivesOnEquip = RAW_RemoveRepeatedSemicolon(item.PassivesOnEquip .. ";RAW_Attunement")
